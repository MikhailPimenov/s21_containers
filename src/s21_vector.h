...
template <typename Vector, typename Pointer, typename Reference,
          typename Difference_type, typename Value_type>
class VectorIteratorBase {
 public:
  ...
  template <typename OtherVector, typename OtherPointer,
            typename OtherReference, typename OtherDifference_type,
            typename OtherValue_type>
  friend class VectorIteratorBase;  // to be able to compare iterator and
                                    // const_iterator
 ...
 public:
  ...
  friend void swap(VectorIteratorBase& left,
                   VectorIteratorBase& right) noexcept {
...
template <typename Vector, typename Pointer = typename Vector::pointer,
          typename Reference = typename Vector::reference>
class VectorIterator
    : public VectorIteratorBase<Vector, Pointer, Reference,
                                typename Vector::difference_type,
                                typename Vector::value_type> {
 private:
  using Base = VectorIteratorBase<Vector, Pointer, Reference,
                                  typename Vector::difference_type,
                                  typename Vector::value_type>;
 ...
 public:
  ...
  template <typename PointerType,
            typename ReferenceType>  // to compare const_iterator with iterator
  difference_type operator-(
      const VectorIterator<Vector, PointerType, ReferenceType>& other)
      const noexcept {
  ...
  bool operator<(const VectorIterator<Vector, PointerType, ReferenceType>&
  ...
  template <typename OtherPointer, typename OtherReference>
  explicit operator VectorIterator<Vector, OtherPointer, OtherReference>()
      const noexcept {
    return VectorIterator<Vector, OtherPointer, OtherReference>(
        const_cast<OtherPointer>(this->pointer_));
  }
};
...
class VectorReverseIterator
 ...
 public:
  ...
  template <typename PointerType, typename ReferenceType>
  difference_type operator-(
      const VectorReverseIterator<Vector, PointerType, ReferenceType>& other)
      const noexcept {
  ...
  // to enable creating iterator from const_iterator via static_cast
  template <typename OtherPointer, typename OtherReference>
  explicit operator VectorReverseIterator<Vector, OtherPointer,
                                          OtherReference>() const noexcept {
    return VectorReverseIterator<Vector, OtherPointer, OtherReference>(
        const_cast<OtherPointer>(this->pointer_));
  }
};
...
class Vector {
 ...
 public:
  ...
  using const_iterator =
      VectorIterator<Vector<Type>, const_pointer, const_reference>;
  using reverse_iterator = VectorReverseIterator<Vector<Type> >;
  using const_reverse_iterator =
      VectorReverseIterator<Vector<Type>, const_pointer, const_reference>;
 ...
 public:
  ...
  constexpr reference Front() { return data_[0ull]; }
  constexpr const_reference Front() const { return data_[0ull]; }

  constexpr reference Back() { return data_[size_ - 1ull]; }
  constexpr const_reference Back() const { return data_[size_ - 1ull]; }
  ...
  reverse_iterator rbegin() { return reverse_iterator(data_ + size_ - 1ull); }
  const_reverse_iterator rbegin() const { return crbegin(); }
  const_reverse_iterator crbegin() const {
    return const_reverse_iterator(data_ + size_ - 1ull);
  }
  ...
  reverse_iterator rend() { return reverse_iterator(data_ - 1ull); }
  const_reverse_iterator rend() const { return crend(); }
  const_reverse_iterator crend() const {
    return const_reverse_iterator(data_ - 1ull);
  }
 ...
 public:
  ...
  constexpr iterator Insert(const_iterator pos, value_type&& value) {
    const size_type new_size = size_ + 1ull;
    checkReallocateUpdatingIterator(new_size, pos);
    auto it = shiftBack(1ull, pos);
 ...
 public:
  ...
  constexpr void Resize(size_type count, const_reference value) {
    ...
    for (size_type i = size_; i < count; ++i) Push_back(value);
 ...
 private:
  ...
  void checkReallocateUpdatingIterator(size_type new_size,
                                       const_iterator& pos) {
    if (new_size > capacity_) {
  ...
  static size_type getDistance(InputIt first, InputIt last) noexcept {
    ...
    if constexpr (std::is_same_v<typename InputIt::iterator_category,
